...
> Шаблон `article` используется для статей опубликованных в периодических изданиях —  научных журналах, газетах и др.

^29cdd8

> [!info] Описание полей
> `type` — тип (`article`), выставлен автоматически
> **Обязательные поля**
> `author` — автор(ы) статьи
> `title` — название (заголовок) статьи
> `journal` — название журнала (издания), где опубликована статья
> `year` — год публикации
> **Стандартные необязательные поля**
> `volume` — том журнала
> `number` — номер журнала
> `pages` — страницы, на которых напечатана статья
> **Нестандартные поля**
> `subtitle` — подзаголовок статьи (встречается редко)
> `month` — месяц публикации статьи
> `note` — дополнительная информация
> `url` — ссылка на сайт, где найдена статья
