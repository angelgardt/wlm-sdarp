...
sim_cov %>% 
  summarise(
    ...
    `$$\\text{var} \\,X$$__a` = var(x1),
    `$$\\text{var} \\,Y$$__a` = var(y1),
    `$$\\text{var} \\,X$$__b` = var(x2),
    `$$\\text{var} \\,Y$$__b` = var(y2),
    `$$\\text{var} \\,X$$__c` = var(x3),
    `$$\\text{var} \\,Y$$__c` = var(y3),
    `$$\\text{var} \\,X$$__d` = var(x4),
    `$$\\text{var} \\,Y$$__d` = var(y4),
    `$$\\text{cov} (X,Y)$$__a` = cov(x1, y1),
    `$$\\text{cov} (X,Y)$$__b` = cov(x2, y2),
    `$$\\text{cov} (X,Y)$$__c` = cov(x3, y3),
    `$$\\text{cov} (X,Y)$$__d` = cov(x4, y4)
  ) %>% 
